...
*/
async function cargaNodo(nivel, direntOrigen, origen, destino) {
 ...
 for (const dirent of dirents) {
  ...
  if (dirent.isDirectory()) {
   const nodo = await cargaNodo(nivel + 1, dirent, nuevoOrigen, nuevoDestino)
   listado.push(nodo)
  } else if (nuevoOrigen.endsWith(".html")) {
   /** @type {{
    *     orden?: string,
   ...
   let json = await leeArchivoJson(sinExtension(nuevoOrigen) + JSON_EXT)
   listado.push({
    i: -1,
    nivel,
    origen: nuevoOrigen,
  ...
  *     description?: string,
  *     printDescription?: string}} */
 let json = await leeArchivoJson(join(origen, INDEX_JSON_FILE))
 return {
  i: -1,
  nivel,
  origen,
...
function creaNavHtml(nodo, paginaAnterior, paginaSiguiente) {
 ...
 if (paginaAnterior && paginaAnterior.ruta !== INDEX_RAIZ_FILE) {
  if (paginaAnterior.orden) {
   encabezado += /* html */
    `<p>
    <a class="paginaAnterior" href="${paginaAnterior.ruta}"
 ...
 if (len > 0) {
  if (listado[0].ruta === INDEX_RAIZ_FILE) {
   ...
   if (p.orden) {
    encabezado += /* html */
     `
     <h1><a href="${p.ruta}">${p.orden}. ${p.h1}</a></h1>`
   } else {
    encabezado += /* html */
     `
     <h1><a href="${p.ruta}">${p.h1}</a></h1>`
   }
  }
 }
 ...
 if (paginaSiguiente) {
  extendida += /* html */
   `<p>
    <a class="paginaSiguiente" href="${paginaSiguiente.ruta}"
